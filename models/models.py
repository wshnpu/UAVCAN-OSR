...
class classifier1D32(nn.Module):
    def __init__(self, num_classes=10):
        super(self.__class__, self).__init__()
        self.num_classes = num_classes

        self.conv1 = nn.Conv1d(1, 64, 3, 1, 1, bias=False)
        self.conv2 = nn.Conv1d(64, 64, 3, 1, 1, bias=False)
        self.conv3 = nn.Conv1d(64, 128, 3, 2, 1, bias=False)

        self.conv4 = nn.Conv1d(128, 128, 3, 1, 1, bias=False)
        self.conv5 = nn.Conv1d(128, 128, 3, 1, 1, bias=False)
        self.conv6 = nn.Conv1d(128, 128, 3, 2, 1, bias=False)

        self.conv7 = nn.Conv1d(128, 128, 3, 1, 1, bias=False)
        self.conv8 = nn.Conv1d(128, 128, 3, 1, 1, bias=False)
        self.conv9 = nn.Conv1d(128, 128, 3, 2, 1, bias=False)
...
class classifierTPBranch(nn.Module):
    def __init__(self, num_classes=10):
        super(self.__class__, self).__init__()
        self.num_classes = num_classes

        self.temp_fc1 = nn.Linear(2, 32, bias=False)
        self.temp_bn1 = nn.BatchNorm1d(32)
        self.temp_fc2 = nn.Linear(32, 64, bias=False)
        self.temp_bn2 = nn.BatchNorm1d(64)

        self.payload_fc1 = nn.Linear(64, 128, bias=False)
        self.payload_bn1 = nn.BatchNorm1d(128)
        self.payload_fc2 = nn.Linear(128, 128, bias=False)
        self.payload_bn2 = nn.BatchNorm1d(128)

        self.fusion_fc1 = nn.Linear(64 + 128, 128, bias=False)
        self.fusion_bn1 = nn.BatchNorm1d(128)
        self.fc = nn.Linear(128, num_classes, bias=False)

        self.temp_dr = nn.Dropout(0.2)
        self.payload_dr = nn.Dropout(0.2)
        self.fusion_dr = nn.Dropout(0.2)
    ...
    def forward(self, x, return_feature=False):
        ...
        time_x = self.temp_dr(time_x)
        time_x = self.temp_fc1(time_x)
        time_x = self.temp_bn1(time_x)
        time_x = nn.LeakyReLU(0.2)(time_x)
        time_x = self.temp_fc2(time_x)
        time_x = self.temp_bn2(time_x)
        time_x = nn.LeakyReLU(0.2)(time_x)

        payload_x = self.payload_dr(payload_x)
        payload_x = self.payload_fc1(payload_x)
        payload_x = self.payload_bn1(payload_x)
        payload_x = nn.LeakyReLU(0.2)(payload_x)
        payload_x = self.payload_fc2(payload_x)
        payload_x = self.payload_bn2(payload_x)
        payload_x = nn.LeakyReLU(0.2)(payload_x)

        x = torch.cat([time_x, payload_x], dim=1)
        x = self.fusion_dr(x)
        x = self.fusion_fc1(x)
        x = self.fusion_bn1(x)
        x = nn.LeakyReLU(0.2)(x)
